...
static zmq_msg_t* do_read(JNIEnv *env, jobject obj, zmq_msg_t *message,
		int flags);
...
#if ZMQ_VERSION >= ZMQ_MAKE_VERSION(3,0,0)
static jboolean s_zerocopy_init(JNIEnv *env, zmq_msg_t *message, jobject obj,
		jint length) {
...
JNIEXPORT jboolean JNICALL Java_org_zeromq_ZMQ_00024Socket_send(JNIEnv *env,
		jobject obj, jbyteArray msg, jint offset, jint length, jint flags) {
	void *s = get_socket(env, obj);
...
inline void setByteBufferPosition(JNIEnv *env, jobject buffer, jint position) {
	...
	jmethodID positionHandle = env->GetMethodID(cls, "position",
			"(I)Ljava/nio/Buffer;");
...
JNIEXPORT jint JNICALL Java_org_zeromq_ZMQ_00024Socket_recv___3BIII(JNIEnv *env,
		jobject obj, jbyteArray buff, jint offset, jint len, jint flags) {
	zmq_msg_t message;
...
JNIEXPORT jboolean JNICALL Java_org_zeromq_ZMQ_00024Socket_monitor(JNIEnv *env,
		jobject obj, jstring addr, jint events) {
#if ZMQ_VERSION >= ZMQ_MAKE_VERSION(3,2,0)
	void *socket = get_socket(env, obj);
...
inline jint get_jbyteArray_length(JNIEnv *env, jbyteArray javaByteArray) {
	return env->GetArrayLength(javaByteArray);
}

inline jint get_java_msg_routing_id(JNIEnv *env, jobject msg) {
	jclass c = env->FindClass("org/zeromq/ZMQ$Msg");
	jmethodID mid = env->GetMethodID(c, "getRoutingId", "()I");
	return env->CallIntMethod(msg, mid);
}

inline jint get_java_msg_offset(JNIEnv *env, jobject msg) {
	jclass c = env->FindClass("org/zeromq/ZMQ$Msg");
	jmethodID mid = env->GetMethodID(c, "getOffset", "()I");
	return env->CallIntMethod(msg, mid);
}

inline jint get_java_msg_length(JNIEnv *env, jobject msg) {
	jclass c = env->FindClass("org/zeromq/ZMQ$Msg");
	jmethodID mid = env->GetMethodID(c, "getLength", "()I");
	return env->CallIntMethod(msg, mid);
}

inline jbyteArray get_java_msg_data(JNIEnv *env, jobject msg) {
	jclass c = env->FindClass("org/zeromq/ZMQ$Msg");
	jmethodID mid = env->GetMethodID(c, "getData", "()[B");
	return (jbyteArray) env->CallObjectMethod(msg, mid);
}

inline jobject create_java_msg(JNIEnv *env, jint routingId, jbyteArray data) {

	jclass c = env->FindClass("org/zeromq/ZMQ$Msg");
	jmethodID cnstrctr = env->GetMethodID(c, "<init>", "()V");

	jobject msg = env->NewObject(c, cnstrctr);
	jmethodID mid;

	mid = env->GetMethodID(c, "setRoutingId", "(I)V");
	env->CallVoidMethod(msg, mid, routingId);

	mid = env->GetMethodID(c, "setData", "([B)V");
	env->CallVoidMethod(msg, mid, data);

	return msg;
}

JNIEXPORT jboolean JNICALL Java_org_zeromq_ZMQ_00024Socket_sendMsg(JNIEnv *env,
		jobject obj, jobject msg, jint flags) {
	void *s = get_socket(env, obj);

	jint routingId = get_java_msg_routing_id(env, msg);
	jbyteArray data = get_java_msg_data(env, msg);
	jint length = get_java_msg_length(env, msg);
	jint offset = get_java_msg_offset(env, msg);
	...
	void* pd = zmq_msg_data(&message);
	env->GetByteArrayRegion(data, offset, length, (jbyte*) pd);
	zmq_msg_set_routing_id(&message, (uint32_t) routingId);
...
/**
 * Called by Java's Socket::recvMsg(int flags).
 */
JNIEXPORT jobject JNICALL Java_org_zeromq_ZMQ_00024Socket_recvMsg(JNIEnv *env,
	...
	int routingId = zmq_msg_routing_id(&message);
	...
	return create_java_msg(env, routingId, data);
}
