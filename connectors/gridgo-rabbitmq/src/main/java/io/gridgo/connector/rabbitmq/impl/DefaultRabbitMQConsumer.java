...
public class DefaultRabbitMQConsumer extends AbstractConsumer implements RabbitMQConsumer, RabbitMQChannelSubscriber {
	...
	public DefaultRabbitMQConsumer(@NonNull Connection connection, @NonNull RabbitMQQueueConfig queueConfig) {
	...
	private void response(Message response, BasicProperties props) {
		...
		byte[] body = BArray.newFromSequence(payload.getId(), payload.getHeaders(), payload.getBody()).toBytes();
		try {
			String responseQueue = props.getReplyTo();
			this.channel.basicPublish(responseQueue, null, props, body);
		} catch (IOException e) {
