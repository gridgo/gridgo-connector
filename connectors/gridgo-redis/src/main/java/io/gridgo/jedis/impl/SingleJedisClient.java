...
public class SingleJedisClient extends JedisClient {

	private JedisPool jedisPool = null;

	public SingleJedisClient(@NonNull RedisConfig config) {
		JedisPoolConfig poolConfig = new JedisPoolConfig();
		HostAndPort endpoint = config.getAddress().getFirst();
		
		this.jedisPool = new JedisPool(poolConfig, endpoint.getHost(), endpoint.getPort(), config.getTimeout(),
				config.getPassword());
	}
	...
	protected void onStop() {
		try {
			...
			throw new RuntimeException("Cannot close jedis", e);
		}
	}

	public String set(String key, String value) {
		return executeOnJedis(jedis -> {
	...
	protected <T> T executeOnJedis(Function<Jedis, T> function) {
		try (Jedis jedis = this.getJedis()) {
			return function.apply(jedis);
		}
	}

	protected void executeOnJedis(Consumer<Jedis> consumer) {
		try (Jedis jedis = this.getJedis()) {
